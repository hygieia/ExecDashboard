...
export class OpenSourceViolationsSegmentationStrategy implements Strategy<MetricSummary, MetricSegmentationModel> {
  parse(model: MetricSummary): MetricSegmentationModel {
    ...
    const rows = [
      { name: 'Total Time', order: 1, accurateName: 'Total Time', unit: 'd' },
      { name: 'Total Items', order: 3, accurateName: 'Total Stories', unit: '' },
      { name: 'Story Points', order: 2, accurateName: 'Total Story Points', unit: '' },
      { name: 'User Stories', order: 4, accurateName: 'Total User Stories', unit: '' },
      { name: 'Enhancements', order: 5, accurateName: 'Total Enhancements', unit: '' },
      { name: 'New Features', order: 6, accurateName: 'Total New Features', unit: '' }
    ];
    ...
    function segmentCount(type: string) {
      //    console.log('segment model: ' + this.model.counts);
      return model.counts
