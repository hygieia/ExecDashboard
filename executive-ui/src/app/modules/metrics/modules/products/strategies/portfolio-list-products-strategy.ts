...
@Injectable()
export class PortfolioListProductsStrategy extends BuildingBlocksStrategyBase {
  ...
  parse(model: BuildingBlockMetricSummary[]): BuildingBlockModel[] {
    ...
    model.forEach((p) => {
      products.push({
        ...
        lob: p.lob,
        poc: p.poc,
        reporting: p.totalComponents,
        total: p.totalExpectedMetrics,
      ...
      .map((metricData) => {
        if (metric.buildingBlockPrimaryMetricStrategy) {
          ...
          return {
            ...
            available: metricData.dataAvailable == undefined ? true : metricData.dataAvailable,
            message: metricData.confMessage
          };          
        }
      });
