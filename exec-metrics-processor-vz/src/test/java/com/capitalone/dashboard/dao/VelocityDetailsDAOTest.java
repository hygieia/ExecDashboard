...
public class VelocityDetailsDAOTest {

	@InjectMocks
	private VelocityDetailsDAO velocityDetailsDAOTest;
	@Mock
	...
	public void testGetMongoClient() throws Exception {
		...
		velocityDetailsDAOTest.getMongoClient();
	}

	@Test
	public void testGetMongoClient_1() {
		Mockito.when(velocityDetailsDAOTest.getMongoClient()).thenThrow(new NullPointerException());
		velocityDetailsDAOTest.getMongoClient();
	}
	...
	public void testGetUserStoriesList() throws Exception {
		...
		basicQuery.addCriteria(new Criteria().andOperator(Criteria.where("sTeamName").is("DevopsCup"),
				...
				Criteria.where("sTypeName").in("VZAgile Story", "Story")));
		...
		Mockito.when(velocityDetailsDAOTest.getMongoClient()).thenReturn(mongoClient);
		velocityDetailsDAOTest.getUserStoriesList("DevopsCup", 254798714, 254798124, mongoClient);
	}

	@Test
	public void testGetUserStoryList() throws Exception {
		List<FeatureUserStory> featureUserStoryList = new ArrayList<>();
		FeatureUserStory featureUserStory = new FeatureUserStory();
		featureUserStory.setsTeamName("DevopsCup");
		featureUserStory.setsTypeName("Story");
		...
		basicQuery.addCriteria(new Criteria().andOperator(Criteria.where("sTeamName").is("DevopsCup"),
				Criteria.where("creationDate").gte(254798714).lte(254798124),
				Criteria.where("sTypeName").in("VZAgile Story")));
		MongoClient mongoClient = mock(MongoClient.class);
		// Mockito.when(mongoOps.find(basicQuery,
		// FeatureUserStory.class)).thenReturn(featureUserStoryList);
		Mockito.when(metricsProcessorConfig.metricsProcessorTemplate(mongoClient)).thenReturn(mongoOps);
		Mockito.when(velocityDetailsDAOTest.getMongoClient()).thenReturn(mongoClient);
		velocityDetailsDAOTest.getUserStoryList("DevopsCup", 254798714, 254798124, mongoClient);
	}
	...
	public void testGetLatestUserStorySorted() throws Exception {
		...
		velocityDetailsDAOTest.getLatestUserStorySorted("DevopsCup", mongoClient);
	}

	@Test
	public void testIsDashboardAvailable_1() throws Exception {
		Mockito.when(velocityDetailsDAOTest.getUserStoriesList("DevopsCup", 254798714, 254798124, mongoClient))
				.thenThrow(new NullPointerException());
		velocityDetailsDAOTest.getUserStoriesList("DevopsCup", 254798714, 254798124, mongoClient);
	}

	@Test
	public void testIsDashboardAvailable_2() throws Exception {
		Mockito.when(velocityDetailsDAOTest.getUserStoryList("DevopsCup", 254798714, 254798124, mongoClient))
				.thenThrow(new NullPointerException());
		velocityDetailsDAOTest.getUserStoryList("DevopsCup", 254798714, 254798124, mongoClient);
	}
	...
	public void testGetEntireAppList() throws Exception {
		Mockito.when(metricsProcessorConfig.metricsProcessorTemplate(mongoClient)).thenReturn(mongoOps);
		velocityDetailsDAOTest.getEntireAppList(mongoClient);
	}
	...
	public void testGetEntireProjectList() throws Exception {
		...
		Mockito.when(metricsProcessorConfig.metricsProcessorTemplate(mongoClient)).thenReturn(mongoOps);
		Mockito.when(velocityDetailsDAOTest.getMongoClient()).thenReturn(mongoClient);
		velocityDetailsDAOTest.getEntireProjectList(mongoClient, "B6LV");
	}

	@Test
	public void tetsprojectsList() throws Exception {
		...
		Mockito.when(metricsProcessorConfig.metricsProcessorTemplate(mongoClient)).thenReturn(mongoOps);
		velocityDetailsDAOTest.getTotalList(mongoClient);
	}

	@Test
	public void tetsGetEntireProjectList_1() throws Exception {
		Mockito.when(velocityDetailsDAOTest.getEntireProjectList(mongoClient, "B6LV"))
				.thenThrow(new NullPointerException());
		velocityDetailsDAOTest.getEntireProjectList(mongoClient, "B6LV");
	}
